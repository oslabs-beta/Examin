...
`
            ...
            currentProps[key] = `mock${currentProps[key]}`;
          }
        }
        // Push the current element's props into the describeBlock
        describeBlockArray.push(`
  let ${componentData[i].name}Props = ${currentProps}
`
);
        // Section for formatting the currentProps in describeBlock
